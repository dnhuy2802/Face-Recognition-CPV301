...
def center_crop(img, percent=0.5):
...
def crop_image_from_center(data_path):
    """
...
crop_image_from_center(TEMP_DATA_DIR)
...
for folder in os.listdir(TEMP_DATA_DIR):
    for file in os.listdir(os.path.join(TEMP_DATA_DIR, folder)):
        ...
        face, bboxes = detectFaceOpenCVDnn(net, img)
        if face is not None:
