...
class VGGModel:
    ...
    def model_create(self):
        ...
        early_stopping = EarlyStopping(monitor='val_loss',
                                       ...
                                       patience=3,
                                       verbose=1)
        checkpoint = ModelCheckpoint(os.path.join(ASSETS_DIR, "vgg_model.keras"),
                                     monitor='val_loss',
                                     save_best_only=True,
                                     mode='min',
                                     verbose=1)
    ...
    def model_train(self):
        ...
        model.fit(train_generator,
                  steps_per_epoch=self.num_train_samples // DEFAULT_BATCH_SIZE,
                  epochs=DEFAULT_EPOCHS,
                  validation_data=validation_generator,
                  callbacks=callbacks)

        # Save the trained model
        model.save(os.path.join(ASSETS_DIR, "vgg_model.keras"),
...
class FaceRecognitionVGGModel:
    ...
    def face_predict(self):
        ...
        # Load the model
        model = load_model(os.path.join(ASSETS_DIR, "vgg_model.keras"))

        # Load the face detector
        modelFile = os.path.join(
            ASSETS_DIR, "res10_300x300_ssd_iter_140000_fp16.caffemodel")
        configFile = os.path.join(ASSETS_DIR, "deploy.prototxt")
