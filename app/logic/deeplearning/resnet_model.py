...
class ResNetModel:
    ...
    def model_create(self):
        ...
        early_stopping = EarlyStopping(monitor='val_loss',
                                    ...
                                    patience=3,
                                    verbose=1)
        checkpoint = ModelCheckpoint(os.path.join(MODEL_DIR, "resnet_model.keras"),
                                    monitor='val_loss',
                                    save_best_only=True,
                                    mode='min',
                                    verbose=1)
    ...
    def model_train(self):
        ...
        model.save(os.path.join(MODEL_DIR,"resnet_model.keras"), overwrite=True, save_format="keras")

        # Evaluate the model on the test data
        scores = model.evaluate(test_generator, verbose=1)
        print("Accuracy: %.2f%%" % (scores[1] * 100))

class FaceRecognitionResNetModel:
    def __init__(self):
        self.people_lables = list()
    ...
    def face_predict(self):
        ...
        # Load the model
        model = load_model(os.path.join(MODEL_DIR, "resnet_model.keras"))
